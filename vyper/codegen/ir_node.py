...
class IRnode:
    ...
    def ir_var(self, name):
        self.cache_levels = getattr(self, 'cache_levels', [])

        if self._optimized.is_complex_ir:
            self.cache_levels.append(False)
            ir_var = IRnode.from_list(
                name, typ=self.typ, location=self.location, encoding=self.encoding)
            ir_var.parent = self
            ir_var.cache_levels = self.cache_levels
            return ir_var

        self.cache_levels.append(True)
        return self

    def resolve(self, body):
        cache_level = self.cache_levels.pop(-1)
        if cache_level:
            return body

        parent = getattr(self, 'parent', self)
        ret = ["with", self, parent, body]
        if isinstance(body, IRnode):
            return IRnode.from_list(
                ret, typ=body.typ, location=body.location, encoding=body.encoding
            )
